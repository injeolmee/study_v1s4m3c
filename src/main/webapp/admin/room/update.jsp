...
<script type="text/JavaScript">
  window.onload=function(){
    CKEDITOR.replace('rocontent');  // <TEXTAREA>태그 id 값
    
    //*********** CKEDITOR에 대한 내용을 받아와 출력 *************
    var content = '${roomVO.rocontent}';
    // alert(content)
    CKEDITOR.instances.rocontent.setData(content);
    //****************************************************************
  ...
  function update_check() {
    ...
    var rofile1 = $('#rofile1').val();
    var rosize1 = $('#rosize1').val();
    var rothumb = $('#rothumb').val();
    var rono = $('#rono').val();// '${param.rono}';
    //alert("파일명 받아오는지 확인: " + rofile1);
 ...
 function update_file(formData) { 
   $.ajax({
     url: "/study/admin/room/update.do",
     type: "POST",
     data: formData,
     dataType: "JSON",
...
//*******************************************************************************
 ...
 function replaceAll(str, searchStr, replaceStr) {
  ...
  //********************************************* 주소 함수 *********************************************
  var list=['서울시', '경기도', '인천', '강원도', '경남', '경북', '광주', '대구', '대전', '부산광역시', '세종', '울산', 
               '전남', '전북', '제주', '충남', '충북'];
  ...
  var gyeonggi =['가평군', '고양시덕양구', '고양시일산동구', '고양시일산서구', '과천시', '광명시', '광주시', '구리시', 
                         '군포시', '김포시', '남양주시', '동두천시', '부천시소사구', '부천시오정구', '부천시원미구', '성남시분당구', 
                         '성남시수정구', '성남시중원구', '수원시영통구', '수원시권선구', '수원시장안구', '수원시팔달구', '시흥시', 
                         '안산시단원구', '안산시상록구', '안성시', '안양시동안구', '안양시만안구', '양주시', '양평군', '여주군', '연천군', 
                         '오산시', '용인시기흥구', '용인시수지구', '용인시처인구', '의왕시', '의정부시', '이천시', '파주시', '평택시', 
                         '포천시', '하남시', '화성시'];
  var incheon = ['강화군', '계양구', '남구', '남동구', '동구', '부평구', '서구', '연수구', '옹진군', '중구'];
  var kangwon = ['고성군', '동해시', '삼척시', '속초시', '양구군', '양양군', '영월군', '원주시', '인제군', '정선군', 
                          '철원군', '춘천시', '태백시', '평창군', '홍천군', '화천군', '횡성군'];
  var gyeongnam = ['거제시', '거창군', '고성군', '김해시', '남해군', '밀양시', '사천시', '산청군', '양산시', '의령군', '진주시', '창녕군', 
                             '창원시', '마산합포구', '창원시', '마산회원구', '창원시', '성산구', '창원시', '의창구', '창원시', '진해구', '통영시', 
                             '하동군', '함안군', '함양군', '합천군'];
  var gyeongbuk = ['경산시', '경주시', '고령군', '구미시', '군위군', '김천시', '문경시', '봉화군', '상주시', '성주군', '안동시', '영덕군', 
                             '영양군', '영주시', '영천시', '예천군', '울릉군', '울진군', '의성군', '청도군', '청송군', '칠곡군', '포항시남구', '포항시북구'];
  ...
  var jeonbuk = ['고창군', '군산시', '김제시', '남원시', '무주군', '부안군', '순창군', '완주군', '익산시', '임실군', 
                        '장수군', '전주시', '덕진구', '전주시', '완산구', '정읍시', '진안군'];
  var jeju = ['서귀포시', '제주시'];
  var chungnam = ['계룡시', '공주시', '금산군', '논산시', '당진시', '보령시', '부여군', '서산시', '서천군', '아산시', 
                            '예산군', '천안시동남구', '천안시서북구', '청양군', '태안군', '홍성군'];
  var chungbuk = ['괴산군', '단양군', '보은군', '영동군', '옥천군', '음성군', '제천시', '증평군', '진천군', '청원군', 
                           '청주시', '상당구', '청주시', '흥덕구', '충주시'];
  
  $('#rocity option').each(function(){ //지역 값 select option 주기
    if($(this).val()=="${roomVO.rocity}"){
      $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
    }
  }); 

  $('#rocity option:selected').each(function(){      
    ...
    $('#rogu option').each(function(){
      if($(this).val()=="${roomVO.rogu}"){
        $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
      } else {
...
<body>
  ...
  <div class="container">
    <DIV class='content' style='width: 60%; margin: 0px auto;'>
      ...
      <DIV class="title_line" style='width: 50%; margin: 50px auto; font-size: 20px; font-weight: bold;'>스터디룸 수정</DIV>
      ...
      <DIV style="text-align: center; width: 100%;">
        ...
        <FORM name='frm' id="frm" method='POST' enctype="multipart/form-data" class="form-horizontal" style="text-align: left;">
          ...
          <input type='hidden' name='rono' id='rono' value='${roomVO.rono }'>
          <input type='hidden' name='rofile1' id='rofile1' value='${roomVO.rofile1}'>
          <input type='hidden' name='rosize1' id='rosize1' value='${roomVO.rosize1}'>
          <input type='hidden' name='rothumb' id='rothumb' value='${roomVO.rothumb}'>
          ...
          <div class="form-group">
            <div class="col-md-10" style="width: 100%; margin-top: 40px;">
              ...
              <input type='text' class="form-control input-md" name='roname' id='roname' value='솔데스크 24시' required="required" style='width: 85%; float: left;'>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="col-md-10" style="width: 100%; margin-top: 10px;">
              ...
              <input type='text' class="form-control input-md" name='rodong' id='rodong' value='${roomVO.rodong }' style='width: 44%; float: left;'>
            </div>
          </div>
          ...
          <div class="form-group" style="margin-bottom: 0px;">
            <div class="col-md-10" style="width: 100%; margin-top: 10px;">
              ...
              <TEXTAREA class="form-control input-md" name='romap' id='romap' rows='5' placeholder="Daum map" style='width: 84%; float: left;'>${roomVO.romap} </textarea>
            </div>
            <DIV style="padding: 3px 0px 10px 15px; text-align: left; letter-spacing: -1px; margin-left: 7%; width: 84%; 
                    background: url(./images/bul_exclamation.png) no-repeat; color: #777; font-size: 11px; line-height: 1; 
                    vertical-align: top; clear: both;">
              <a href="http://map.daum.net/" target="_blank">http://map.daum.net/</a>
              (순서 : 주소검색 -> 상세보기 -> 우측상단 깃발 클릭 -> 지도크기 선택시 사용자 지정을 선택 후 520 * 300으로 설정 -> 소스코드생성)
            </DIV>
          </div>
          ...
          <div class="form-group">
            <div class="col-md-10" style="text-align: center; width: 95%; margin-top: 0px;">
              <textarea class="form-control input-md" name='rocontent'  id='rocontent' rows='20' cols='70'>${roomVO.rocontent }</textarea>
            </div>
          </div>
          ...
          <div class="form-group form-group-sm" style='margin-bottom: 10px;'>
            <div class="col-md-10" style="width: 100%; margin-top: 0px;">
              ...
              <input type='text' class="form-control input-lg" placeholder="전화번호" name='rotel' id='rotel'  value='${roomVO.rotel}' style='width: 26.3%; float: left; margin-right: 6px;'>
              <input type='text' class="form-control input-lg" placeholder="홈페이지" name='rosite' id='rosite' value='${roomVO.rosite}'  style='width: 26.3%; float: left; margin-right: 6px;'>
              <input type='text' class="form-control input-lg" placeholder="운영시간" name='rorunday' id='rorunday' value='${roomVO.rorunday}'  style='width: 26.3%; float: left;'>
            </div>
          </div>
          ...
          <div class="form-group form-group-sm" style='margin-bottom: 10px;'>
            <div class="col-md-10" style="width: 100%; margin-top: 0px;">
              ...
              <input type='text' class="form-control input-lg" placeholder="비용: 예) 1인당 1주문" name='rocost' id='rocost' value='${roomVO.rocost}'  style='width: 41%; float: left; margin-right: 6px;'>
              <input type='text' class="form-control input-lg" placeholder="룸 정보: 예) 1호실 4명" name='rocount' id='rocount' value='${roomVO.rocount}'  style='width: 41%; float: left;'>
            </div>
          </div>
          ...
          <div class="form-group" style="width: 500px; display: flex;">
            <label for="file1MF" class="col-md-2 control-label"  style="width: 22%; padding-top: 2px;">기존 파일 </label>
            <div class="col-md-10"  style="text-align: center; width: 100%;">
              ...
              <c:choose>
                <c:when test="${fn:endsWith(rofile1, '.jpg')}">
                  <IMG id='rofile1' src='./storage/${roomVO.rofile1}' style='width: 20%;'>
                </c:when>
                ...
                <c:when test="${roomVO.rofile1.length() > 0}">
                  ${roomVO.rofile1 }  <!-- 이미지가 아니면서 파일이 존재하는 경우 파일명 출력 -->
                </c:when>
              </c:choose>
            </div>
          </div>
