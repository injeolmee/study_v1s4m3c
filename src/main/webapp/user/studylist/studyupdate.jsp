...
$(function(){
  ...
  var dow1 = "${studyListVO.dow1}";
  var dow2 = "${studyListVO.dow2}";
  var dow3 = "${studyListVO.dow3}";
  var dow4 = "${studyListVO.dow4}";
  var dow5 = "${studyListVO.dow5}";
  var dow6 = "${studyListVO.dow6}";
  var dow7 = "${studyListVO.dow7}";
  
  $("#dow1").each(function(){
    if(dow1 == "월") {
      $('#dow1').prop("checked", true );
    }
  });  
  $("#dow2").each(function(){
    if(dow2 == "화") {
      $('#dow2').prop("checked", true );
    }
  });
  $("#dow3").each(function(){
    if(dow3 == "수") {
      $('#dow3').prop("checked", true );
    }
  });
  $("#dow4").each(function(){
    if(dow4 == "목") {
      $('#dow4').prop("checked", true );
    }
  });
  $("#dow5").each(function(){
    if(dow5 == "금") {
      $('#dow5').prop("checked", true );
    }
  });
  $("#dow6").each(function(){
    if(dow6 == "토") {
      $('#dow6').prop("checked", true );
    }
  });
  $("#dow7").each(function(){
    if(dow7 == "일") {
      $('#dow7').prop("checked", true );
    }
  });
  ...
  $('#change_email').change(function(){
    ...
    $('#area option').each(function(){ //지역 값 select option 주기
      if($(this).val()=="${studyListVO.area}"){
        $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
      }
    });
    
    $('#stdlist_topic option').each(function(){ //분야 값 select option 주기
      if($(this).val()=="${studyListVO.stdlist_topic}"){
        $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
      }
    });
  ...
  $('#area option:selected').each(function(){      
    ...
    $('#selected_area option').each(function(){
      if($(this).val()=="${studyListVO.selected_area}"){
        $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
      } else {
...
<DIV class='menu_line' ><h2>스터디 그룹 수정</h2></DIV>
 
 <DIV style='width: 60%; margin: 0px auto;'>
 
<FORM name='frm' method='POST' action='./update.do' class="form-horizontal">
  <input type='hidden' name='memberno' id='memberno' value='1'>
  <input type='hidden' name='stdlist_no' id='${studyListVO.stdlist_no}' value='1'>
  
      <div class="form-group" style='padding:30px; margin:0;'>   
        <label for="stdlist_title" class="col-md-1 control-label">제목</label> 
        <div class="col-md-10">
          <input type='text' class="form-control input-md" name='stdlist_title' id='stdlist_title' value='${studyListVO.stdlist_title}' autofocus="autofocus" required="required" placeholder="제목" style="width:70%; height:35px;">
        </div>
      </div>    
      ...
      <div class="form-group" style='padding:30px; margin:0;'>   
        ...
        <div class="col-md-10">
            <input type="text" class="form-control" id="email1" name="email1"  value="${studyListVO.email1}" required="required" placeholder="이메일아이디" style="width:25%;height:35px;" /> @
            <input type="text" class="form-control" id="email2" name="email2"  value="${studyListVO.email2}" required="required" placeholder="이메일주소" style="width:25%;height:35px;" />
      ...
      <div class="form-group" style='padding:30px; margin:0;'>   
        ...
        <div class="col-md-10">
            ...
            <optgroup label="-- 프로그래밍 --">
              <option value="프로그래밍JAVA">JAVA</option>
  ...
  <div class="form-group" style='padding:30px; margin:0;'>
    <label for='stdlist_time' class="col-md-1 control-label">시간</label>
      <div class="col-md-10">
        <input type="text" class="form-control" name='stdlist_time' id='stdlist_time' value='${studyListVO.stdlist_time}' style="width:10%; height:35px; margin-bottom: 10px;">
  ...
  <div class="form-group" style='padding:30px; margin:0;'>
    ...
    <div class="col-md-10">     
      <input type ='number' class="form-control" name='stdlist_tot_num' id='stdlist_tot_num' min='4' max='10' step='1' value='${studyListVO.stdlist_tot_num}' style='width:30%; height:35px; margin-bottom:10px;'>
    </div>   
  </div>
  ...
  <div class="form-group" style='padding:30px; margin:0;'>
    ...
    <div class="col-md-10">
     <textarea class="form-control" rows="30" name='stdlist_content' id="stdlist_content" style='margin-bottom:10px;'>${studyListVO.stdlist_content}</textarea>
    </div>
  </div>  
