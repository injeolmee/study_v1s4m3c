...
$(function(){ 
  ...
  var memgender = "${memberVO.memgender}";
  var mbirthvb = "${memberVO.mbirthvb}";
  var mgendervb = "${memberVO.mgendervb}";
  var maddressvb = "${memberVO.maddressvb}";
  var mphonevb = "${memberVO.mphonevb}";
  var msnsvb = "${memberVO.msnsvb}";
  var mintrovb = "${memberVO.mintrovb}";
  var mphotovb = "${memberVO.mphotovb}";
  ...
  $("input[name='memgender']").each(function(){
    if(memgender == "남"){
  ...
  $("input[name='mbirthvb']").each(function(){
    if(mbirthvb == "N") {
      $('#mbirthvb').prop("checked", true );
    }
  });  
  $("input[name='mgendervb']").each(function(){
    if(mgendervb == "N") {
      $('#mgendervb').prop("checked", true );
    }
  });  
  $("input[name='maddressvb']").each(function(){
    if(maddressvb == "N") {
      $('#maddressvb').prop("checked", true );
    }
  });  
  $("input[name='mphonevb']").each(function(){
    if(mphonevb == "N") {
      $('#mphonevb').prop("checked", true );
    }
  });  
  $("input[name='msnsvb']").each(function(){
    if(msnsvb == "N") {
      $('#msnsvb').prop("checked", true );
    }
  });  
  $("input[name='mintrovb']").each(function(){
    if(mintrovb == "N") {
      $('#mintrovb').prop("checked", true );
    }
  });  
  $("input[name='mphotovb']").each(function(){
    if(mphotovb == "N") {
      $('#mphotovb').prop("checked", true );
    }
  });  
  ...
  /* -------------------------------- 인증 확인 클릭 종료-----------------------------------  */   
   ...
   /* ---------------------------------------------------- 주소 시작------------------------------------------------------------- */
    ...
    $('#area option:selected').each(function(){      
      ...
      $('#selected_area option').each(function(){
        if($(this).val()=="${memberVO.selected_area}"){
          $(this).prop("selected","selected"); // attr적용안될경우 prop으로 
        } else {
...
<FORM name='frm_join' id='frm_join' method='POST' action='./mem_update.do' enctype="multipart/form-data" class="form-horizontal">  
...
<input type="hidden" id="memauth" name="memauth" value="${memberVO.memauth }">
  ...
  <div class="form-group" style= 'padding: 20px;'>
    ...
    <div class="col-sm-7" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="memid" name="memid" value="${memberVO.memid }" style="padding:6px; margin:0px; width: 43%;" disabled="disabled">
    </div>  
  </div>
  ...
  <div class="form-group" style= 'padding: 20px;'>
    ...
    <div class="col-md-3" style='padding-left: 10px;'>
      <input type="password" class="form-control" id="mempasswd_c" name="mempasswd_c">
    </div>
  ...
  <div class="form-group" style= 'padding: 20px;'>
    ...
    <div class="col-md-3" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="mememail" name="mememail" value="${memberVO.mememail }" placeholder="이메일  @ 이메일.com">
    </div>
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-7" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="memname" name="memname" value="${memberVO.memname }" placeholder="홍길동" style="width: 43%; padding:6px; margin:0px;">
    </div>    
  </div>
...
<c:choose>
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-7" style='padding: 10px 0px 5px 10px; text-align: left;'>
      <label class="radio-inline" style= 'padding:0px 0px 5px; margin:0px 30px;'>
        <input type="radio" name="memauth" id="N" value="N" /> 대기</label>        
      <label class="radio-inline" style= 'padding:0px 0px 5px;'>
        <input type="radio" name="memauth" id="U" value="U" /> 회원</label>    
    </div>
  </div>
  </c:when>
  <c:otherwise>
  <div class="form-group" style= 'padding: 20px;'>
    ...
    <div class="col-sm-6" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="memauth" name="memauth" value="${memberVO.memauth }" style='padding: 6px; margin: 0px; width:50%;' disabled="disabled">
    </div>  
  </div>    
  </c:otherwise>
</c:choose>   
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-7" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="membirth" name="membirth" maxlength="8" value="${memberVO.membirth }" placeholder="예) 19990101" style="width: 43%; padding:6px; margin:0px;">
      <label>
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-7" style='padding: 10px 0px 5px 10px; text-align: left;'>
      <label class="radio-inline" style= 'padding:0px 0px 5px; margin:0px 30px;'>
        <input type="radio" name="memgender" id="male" value="남" /> 남자</label>        
      <label class="radio-inline" style= 'padding:0px 0px 5px;'>
        <input type="radio" name="memgender" id="female" value="여" /> 여자</label>
      <label style='margin-left: 28%;'>
        <input type="checkbox" id="mgendervb" name="mgendervb" value="N" style= "margin: 5px;"><strong>비공개</strong>
      </label>
    </div>
  </div> 
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-3" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="memphone" name="memphone" maxlength="11" value="${memberVO.memphone }" placeholder="예) 01012345678" style="padding:6px; margin:0px;">
    </div>
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-3" style='padding-left: 10px;'>
      <input type="text" class="form-control" id="memsns" name="memsns" value="${memberVO.memsns }" placeholder="예) 트위터 홍길동" style="padding:6px; margin:0px;">
    </div>
    <div class="col-sm-4" style='text-align: left; padding: 10px 0px 5px 0px;'>
      <label>
  ...
  <div class="form-group" style='padding: 20px;'>
    ...
    <div class="col-sm-5" style='padding-left: 10px;'>
      <textarea class= 'form-control' rows="3" id="memintro" name="memintro" style='padding:6px;' placeholder="자기소개를 해주세요">${memberVO.memintro }</textarea>
    </div>
...
<%--   <div class="form-group">
    <div class=" col-sm-2" style='background-color: #e4e4e4; text-align: center; padding: 8px;'><strong>미리보기</strong></div>
        <div class="col-sm-2" style='padding: 5px 0px 5px 10px;'>
            <c:set var='memphoto_t' value="${fn:toLowerCase(memberVO.memphoto_t)}" />              
              <c:choose>
                <c:when test="${fn:endsWith(memphoto_t, '.jpg')}">
                  <IMG id='memphoto_t' src='./storage/${memberVO.memphoto_t}' >
                </c:when>
                <c:when test="${fn:endsWith(memphoto_t, '.gif')}">
                  <IMG id='memphoto_t'  src='./storage/${memberVO.memphoto_t}' >
                </c:when>
                <c:when test="${fn:endsWith(memphoto_t, '.png')}">
                  <IMG id='memphoto_t'  src='./storage/${memberVO.memphoto_t}'>
                </c:when>
                <c:otherwise>           
                 <IMG id='memphoto_t' src="../jeimages/none2.png" style='width:110px; height:120px;'>        
                </c:otherwise>
              </c:choose>
    </div>
    <div class="col-sm-4" id="image_store" style='display:none;'>
      <img src="../jeimages/none2.png" style='width: 110px; height:120px; margin-right: 40px;'>
    </div>
    <div class="col-sm-4" id="image_preview" style='padding: 5px 0px 5px 10px; text-align:left;'>      
      <button type="button" class="btn" id="file_remove"><strong>REMOVE</strong></button>
    </div>    
  </div> --%>
        ...
        <c:choose>                               
          ...
          <c:otherwise>
            <div style='width:110px; height:120px;'> 
              <IMG id='image_preview' src="<%=root %>/jeimages/none2.png" style='width:110px; height:120px;'>
            </div>
          </c:otherwise>
      </c:choose>
